...
namespace json {
    ...
    public:
        ...
        explicit Node(nullptr_t) : value_(nullptr) {
        }

        explicit Node(int value) : value_(value) {
        }

        explicit Node(double value) : value_(value) {
        }


        explicit Node(string value) : value_(std::move(value)) {
        }

        explicit Node(const char *value) : value_(string(value)) {
        }

        explicit Node(bool value) : value_(value) {
        }

        explicit Node(Array array) : value_(std::move(array)) {
        }

        explicit Node(Dict dict) : value_(std::move(dict)) {
        }
